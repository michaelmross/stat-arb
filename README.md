...
- `statarb/` — the package (generators, estimators, backtesters, loaders)
- `*.py` at root — drivers, validation studies, and figure generation
- `paper/` — `etf_pairs_note.tex` and its figures
- `*.json` — archived datasets, each regenerated by a named driver
- `data/`, `futures/` — price CSVs (gitignored; see *Data availability*)
...
| `statarb/futures.py` | Production margins from stitched front-month futures: fixed-weight spread definitions, contract calendars and roll masking, N-leg price-space backtester with tick-and-commission costs. |
| `fetch_futures.py` | Fetches the crack/crush/cattle legs from Yahoo; roll-gap diagnostics; retains negative settlements (WTI 2020-04-20). |
| `validate_roll.py` | Ground-truth validation of the roll treatment: ADF size on a random-walk margin seen through stitched contracts, with and without masking. |
| `scan_futures.py` | Tests the production margins under the same discovery/evaluation protocol as the ETF scan. |
...
python kalman_variant.py --costs        # -> kalman_cost_attribution.json
```
...
```bash
python fetch_futures.py --out futures            # 9 legs from Yahoo
python validate_roll.py                          # roll-gap null, ground truth
python scan_futures.py                           # -> futures_results.json
```

The write-up:

```bash
python texcheck.py paper/etf_pairs_note.tex      # structural check
python make_figures.py --out paper               # all five figures
cd paper && pdflatex etf_pairs_note.tex && pdflatex etf_pairs_note.tex
```
...
The write-up is `paper/etf_pairs_note.tex` — *Cost Viability and
Cointegration Are Anti-Correlated in Liquid US ETF Pairs: A
ground-truth-validated negative result for daily-frequency statistical
arbitrage*, Michael M. Ross, 2026. A Zenodo DOI is reserved at tagging;
